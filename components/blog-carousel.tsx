...
export function BlogCarousel() {
  ...
  const fetchBlogs = async () => {
    try {
      ...
      if (res.ok) {
        ...
        setBlogs(data.slice(0, 6)) // Show max 6 blogs
      }
    } catch (error) {
      console.error(`Failed to fetch ${getBlogNamePlural().toLowerCase()}:`, error)
      toast.error(`Failed to load ${getBlogNamePlural().toLowerCase()}`)
    } finally {
      setLoading(false)
    }
  }
  ...
  if (loading) {
    return <div className="text-center py-8">Loading {getBlogNamePlural().toLowerCase()}...</div>
  }

  if (blogs.length === 0) {
    return (
      <div className="text-center py-8">
        <p className="text-muted-foreground">No {getBlogNamePlural().toLowerCase()} available yet.</p>
      </div>
    )
  }
  ...
  return (
    <div className="relative">
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-2xl font-bold">Latest {getBlogNamePlural()}</h2>
